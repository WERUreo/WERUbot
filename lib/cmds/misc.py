...
def help(bot, prefix, cmds):
    bot.send_message(f"Registered commands: " + ", ".join([f"{prefix}{cmd.callables[0]}" for cmd in sorted(cmds, key=lambda cmd: cmd.callables[0])]))
...
def about(bot, user, *args):
    bot.send_message("Version 1.0.0. Developed by WERUreo.")
