...
def warn(bot, user, target=None, *reason):
    if target is None:
        ...
        reason = " ".join(reason)
        warnings = db.field("SELECT Warnings FROM users WHERE UserName = ?", target.lower())
        ...
        if warnings is None:
            ...
            db.execute("UPDATE users SET Warnings = Warnings + 1 WHERE UserName = ?", target)
...
def remove_warn(bot, user, target=None, *args):
    if target is None:
        ...
        if warnings == 0:
            ...
            db.execute("UPDATE users SET Warnings = Warnings - 1 WHERE UserName = ?", target.lower())

            bot.send_message(f"Warning for {target} removed.")